    ...
    <div className="max-w-lg mx-auto p-6 bg-slate-800 border border-slate-700 rounded-2xl text-white shadow-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 font-mono text-xs">
        <div>
          ...
          <input type="text" required placeholder="SENA-1006" value={placaSena} onChange={(e) => setPlacaSena(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-white focus:outline-none focus:border-sena-green" />
        </div>
        <div>
          <label className="block font-bold text-slate-300 mb-1">Marca / Modelo</label>
          <input type="text" required placeholder="Lenovo ThinkPad L14 G3" value={marcaModelo} onChange={(e) => setMarcaModelo(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-white focus:outline-none focus:border-sena-green" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block font-bold text-slate-300 mb-1">Memoria RAM</label>
            <select value={ram} onChange={(e) => setRam(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-white focus:outline-none focus:border-sena-green">
          ...
          <div>
            ...
            <select value={estado} onChange={(e) => setEstado(e.target.value as Equipo['estado'])} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-white focus:outline-none focus:border-sena-green">
        ...
        <div>
          ...
          <input type="text" required value={ambiente} onChange={(e) => setAmbiente(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-white focus:outline-none focus:border-sena-green" />
        </div>
