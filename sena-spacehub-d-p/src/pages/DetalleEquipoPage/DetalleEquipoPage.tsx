...
export default function DetalleEquipoPage() {
  ...
  return (
    <div className="max-w-lg mx-auto p-6 bg-slate-800 border border-slate-700 rounded-2xl text-white shadow-xl">
      ...
      <p className="text-xs text-slate-400 mb-4 font-mono">Placa SENA: <span className="text-white font-bold">{placaSena}</span></p>
      ...
      <form onSubmit={handleUpdate} className="space-y-4 font-mono text-xs">
        ...
        <div>
          ...
          <input type="text" value={ambiente} onChange={(e) => setAmbiente(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-white focus:outline-none focus:border-sena-green" />
        </div>
        <div className="pt-2 flex justify-end gap-3 font-sans">
          ...
          <button type="submit" className="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white font-bold rounded-xl text-xs shadow-lg">Actualizar Recurso (PUT)</button>
        </div>
      </form>
    </div>
  );
